...
// expose globally FIRST
window.$ = window.jQuery = $;

if (!window.jQuery) {
    throw new Error("jQuery not found on window");
}
...
(function ($) {
    ...
    $(function () {
        ...
        $('#homePageForm').on('submit', function (e) {
            e.preventDefault();
            var $form = $(this);
            var name = ($form.find('[name="contactname"]').val());
            var email = ($form.find('[name="contactemail"]').val());
            var mobile = ($form.find('[name="contactphone"]').val());
            var service = ($form.find('#contactservice').val());
            var contactmessage = ($form.find('[name="contactmessage"]').val());
            ...
            if (!name) {
                $form.find('[name="contactname"]').addClass('error');
                hasError = true;
                $(`#contactnameError`).text("Field is requried");
            } else {
                $(`#contactnameError`).text("");
            }

            if (!mobile || !/^\d{10}$/.test(mobile)) {
                $form.find('[name="contactphone"]').addClass('error');
                hasError = true;
                $(`#contactphoneError`).text("Field is requried");
            } else {
                $(`#contactphoneError`).text("");
            }

            if (!email) {
                $form.find('[name="contactemail"]').addClass('error');
                hasError = true;
                $(`#contactemailError`).text("Field is requried");
            } else if (!validateEmail(email)) {
                $(`#contactemailError`).text("Enter a valid email");
            } else {
                $(`#contactemailError`).text("");
            }

            if (!service || service === "Service *") {
                $form.find('[name="contactservice"]').addClass('error');
                hasError = true;
                $(`#contactserviceError`).text("Field is requried");
            } else if (!validateEmail(email)) {
                $(`#contactserviceError`).text("Enter a valid email");
            } else {
                $(`#contactserviceError`).text("");
            }


            if (hasError) {
                // simple shake animation for the form when invalid
                $form.css({ 'position': 'relative' }).animate({ left: -6 }, 40).animate({ left: 6 }, 40).animate({ left: 0 }, 40);
                return;
            }


            // mock submit: you can replace this with an AJAX call to your API
            var payload = {
                name: name,
                email: email,
                mobile: mobile,
            ...
            console.log(JSON.stringify(payload, null, 2), contactmessage);
            ...
            const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbxlg-guV3k2hA0NoCLzOuWQuowyX6N_NBgD8IS9e54Xtrs5b5ehnGYCYuLB2isgfm9ipg/exec';
            ...
            fetch(GOOGLE_SCRIPT_URL, {
                ...
                .then(() => {
                    ...
                    Swal.fire({
                        title: "Thank you!",
                        text: "For reaching out. A member of our team will contact you shortly.",
                        icon: "success",
                        iconColor: "#07244E",
        ...
        // add numeric-only for mobile
        $(document).on('input', '[name="contactphone"]', function () {
